...
Graphify shines when agents ask *relational* questions (“what uses `resolveLogo`?”, “how do domains relate to categories?”, “blast radius of changing the content schema”). It does **not** replace:
...
| **Repomix of *only* `src/content/config.ts` + one example MDX** | Fastest onboarding for content-only tasks |
...
- For *this* repo **today**, bigger wins are usually: **doc progressive disclosure, ignore build output, schema-first skills, and a hard verify loop**.
