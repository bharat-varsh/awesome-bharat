...
function calculateRelevanceScore<K extends readonly (keyof SupportedCollectionData)[]>(
  ...
  for (const field of matchingFields) {
    ...
    if (Array.isArray(currentValue) && Array.isArray(candidateValue)) {
      ...
      const currentHasObjects = currentValue.length > 0 && typeof currentValue[0] === 'object';
      const candidateHasObjects = candidateValue.length > 0 && typeof candidateValue[0] === 'object';
